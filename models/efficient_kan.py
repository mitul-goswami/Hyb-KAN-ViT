...
class EfficientKAN(nn.Module):
    def __init__(self, in_dim, out_dim, grid_size=5, spline_order=3, grid_range=[-1.5, 1.5], scale_base=1.0, scale_spline=1.0):
        super().__init__()
        self.base_weight = nn.Parameter(torch.empty(out_dim, in_dim))
        self.spline_scaler = nn.Parameter(torch.ones(out_dim, in_dim) * scale_spline)
        self.bias = nn.Parameter(torch.zeros(out_dim))
        self.spline_act = BSplineActivation(in_features=in_dim, grid_size=grid_size, spline_order=spline_order, grid_range=grid_range)
